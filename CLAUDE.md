...
- **Menu bar provider**: both data sources may stay enabled, but `AppSettings.menuBarProvider` selects exactly one compact status-item display (`claude` or `codex`). Do not append both providers to the status title.
